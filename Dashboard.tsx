...
export const Dashboard = ({ transactions, accounts, students, masterData, currentUser, setActiveTab }: { transactions: Transaction[], accounts: Account[], students: Student[], masterData: any, currentUser: string, setActiveTab: any }) => {
  ...
  return (
    <div>
      <div style={{marginBottom: '20px', borderBottom: '1px solid #e2e8f0', paddingBottom: '15px', display: 'flex', justifyContent: 'space-between', alignItems: 'center'}}>
         <div>
      ...
      <div style={{display: "grid", gridTemplateColumns: "repeat(5, 1fr)", gap: "20px"}}>
        <div style={styles.kpiCard("#3b82f6", "#eff6ff")}>
           ...
           <div style={{alignSelf: 'flex-end', padding: '8px', background: '#dbeafe', borderRadius: '8px', color: '#3b82f6'}}><span className="material-symbols-outlined">groups</span></div>
        </div>
        <div style={styles.kpiCard("#8b5cf6", "#f5f3ff")}>
           ...
           <div style={{alignSelf: 'flex-end', padding: '8px', background: '#ede9fe', borderRadius: '8px', color: '#8b5cf6'}}><span className="material-symbols-outlined">calendar_month</span></div>
        </div>
        <div style={styles.kpiCard("#f97316", "#ffedd5")}>
           ...
           <div style={{alignSelf: 'flex-end', padding: '8px', background: '#fed7aa', borderRadius: '8px', color: '#f97316'}}><span className="material-symbols-outlined">account_balance</span></div>
        </div>
        <div style={styles.kpiCard("#10b981", "#ecfdf5")}>
           ...
           <div style={{alignSelf: 'flex-end', padding: '8px', background: '#d1fae5', borderRadius: '8px', color: '#10b981'}}><span className="material-symbols-outlined">account_balance_wallet</span></div>
        </div>
        <div style={{...styles.kpiCard("#f43f5e", "#fff1f2"), cursor: 'pointer'}} onClick={() => setShowDailyReport(true)}>
           ...
           <div style={{alignSelf: 'flex-end', padding: '8px', background: '#ffe4e6', borderRadius: '8px', color: '#f43f5e'}}><span className="material-symbols-outlined">description</span></div>
        </div>
      </div>
            ...
            <div style={{...styles.modalContent, width: '850px', padding: '0', overflow: 'hidden'}}>
               <div className="no-print" style={{padding: '20px', borderBottom: '1px solid #e2e8f0', display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: '#f8fafc'}}>
                  ...
                  <div style={{display: 'flex', gap: '10px'}}>
                     <button style={{...styles.button("primary"), background: '#1e293b'}} onClick={handleExportPDF}><span className="material-symbols-outlined">picture_as_pdf</span> Export PDF</button>
               ...
               <div id="daily-report-content" style={{padding: '40px', backgroundColor: 'white'}}>
                  ...
                  <div style={{display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '30px'}}>
                     <div>
                        <h4 style={{borderBottom: '2px solid #166534', paddingBottom: '10px', color: '#166534', display: 'flex', alignItems: 'center', gap: '8px'}}><span className="material-symbols-outlined">arrow_downward</span> Income (Receipts)</h4>
                        <table style={{width: '100%', borderCollapse: 'collapse'}}>
                           <tbody>
                              {Object.entries(incomeAgg).map(([head, amount]) => (
                                  <tr key={head} style={{borderBottom: '1px solid #f1f5f9'}}><td style={{padding: '12px 0', color: '#334155'}}>{head}</td><td style={{textAlign: 'right', padding: '12px 0', fontWeight: 600}}>Rs {amount.toLocaleString()}</td></tr>
                              ))}
                              <tr style={{background: '#f0fdf4'}}><td style={{padding: '12px 10px', fontWeight: 700}}>Total Receipts</td><td style={{textAlign: 'right', padding: '12px 10px', fontWeight: 700, color: '#166534'}}>Rs {totalReceiptsToday.toLocaleString()}</td></tr>
                           </tbody>
                        </table>
                     </div>
                     <div>
                        <h4 style={{borderBottom: '2px solid #b91c1c', paddingBottom: '10px', color: '#b91c1c', display: 'flex', alignItems: 'center', gap: '8px'}}><span className="material-symbols-outlined">arrow_upward</span> Expenses (Payments)</h4>
                        <table style={{width: '100%', borderCollapse: 'collapse'}}>
                           <tbody>
                              {Object.entries(expenseAgg).map(([head, amount]) => (
                                  <tr key={head} style={{borderBottom: '1px solid #f1f5f9'}}><td style={{padding: '12px 0', color: '#334155'}}>{head}</td><td style={{textAlign: 'right', padding: '12px 0', fontWeight: 600}}>Rs {amount.toLocaleString()}</td></tr>
                              ))}
                              <tr style={{background: '#fef2f2'}}><td style={{padding: '12px 10px', fontWeight: 700}}>Total Payments</td><td style={{textAlign: 'right', padding: '12px 10px', fontWeight: 700, color: '#b91c1c'}}>Rs {totalPaymentsToday.toLocaleString()}</td></tr>
                           </tbody>
                        </table>
                     </div>
                  </div>
                  <div style={{marginTop: '40px', padding: '25px', background: '#f8fafc', borderRadius: '12px', border: '1px solid #e2e8f0', display: 'flex', justifyContent: 'space-between', alignItems: 'center'}}>
                     <div><div style={{fontSize: '0.85rem', color: '#64748b', textTransform: 'uppercase', letterSpacing: '0.5px'}}>Net Day Cash Position</div><div style={{fontSize: '1.8rem', fontWeight: 800, color: '#0f172a'}}>Rs {(totalReceiptsToday - totalPaymentsToday).toLocaleString()}</div></div>
                     <div style={{textAlign: 'right'}}><div style={{fontSize: '0.85rem', color: '#64748b', textTransform: 'uppercase', letterSpacing: '0.5px'}}>Closing Cash Balance</div><div style={{fontSize: '1.8rem', fontWeight: 800, color: '#1d4ed8'}}>Rs {cashInHand.toLocaleString()}</div></div>
                  </div>
               </div>
            </div>
         </div>
      ...
      <div style={{marginTop: '30px'}}>
        <h3 style={{display: 'flex', alignItems: 'center', gap: '8px', fontSize: '1rem', color: '#0f172a'}}><span className="material-symbols-outlined" style={{color: '#059669'}}>domain</span> Campus Wise Recovery</h3>
        <div style={{display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '20px', marginTop: '15px'}}>
           {campusStats.map((stat: any) => (
             <div key={stat.campus} style={styles.card}>
                ...
                <div style={{display: 'flex', justifyContent: 'space-between', marginBottom: '8px', fontSize: '0.85rem'}}><span style={{color: '#64748b'}}>Students</span><span style={{fontWeight: 600}}>{stat.students}</span></div>
                <div style={{display: 'flex', justifyContent: 'space-between', marginBottom: '8px', fontSize: '0.85rem'}}><span style={{color: '#64748b'}}>Collected</span><span style={{fontWeight: 600, color: '#059669'}}>Rs {stat.collected.toLocaleString()}</span></div>
                <div style={{display: 'flex', justifyContent: 'space-between', fontSize: '0.85rem'}}><span style={{color: '#64748b'}}>Receivable</span><span style={{fontWeight: 600, color: '#ef4444'}}>Rs {stat.receivable.toLocaleString()}</span></div>
             </div>
           ))}
        </div>
      </div>
      ...
      <div style={{marginTop: '10px', ...styles.card, padding: '30px'}}>
         <div style={{display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '30px'}}>
            <h3 style={{margin: '0', fontSize: '1.1rem', display: 'flex', alignItems: 'center', gap: '10px', color: '#1e293b'}}><span className="material-symbols-outlined" style={{color: '#4f46e5'}}>analytics</span> Annual Recovery Trend ({currentYear})</h3>
         </div>
         <div style={{height: '350px', position: 'relative', width: '100%', display: 'flex', alignItems: 'flex-end', paddingBottom: '30px', paddingLeft: '50px', boxSizing: 'border-box'}}>
            <div style={{position: 'absolute', left: 0, top: 0, bottom: '30px', width: '45px', display: 'flex', flexDirection: 'column-reverse', justifyContent: 'space-between', fontSize: '0.75rem', color: '#94a3b8', textAlign: 'right', paddingRight: '10px'}}>
                <span>0</span><span>{(maxAmount * 0.25).toLocaleString()}</span><span>{(maxAmount * 0.5).toLocaleString()}</span><span>{(maxAmount * 0.75).toLocaleString()}</span><span>{maxAmount.toLocaleString()}</span>
            </div>
                ...
                {graphData.map((d, i) => {
                    ...
                    return (
                        <div key={i} style={{flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', height: '100%', justifyContent: 'flex-end', position: 'relative'}}>
                            <div title={`${d.month}: Rs ${d.amount.toLocaleString()}`} style={{width: '40%', height: `${heightPct}%`, background: 'linear-gradient(to top, #4f46e5, #10b981)', borderRadius: '6px 6px 0 0', transition: 'all 0.3s ease', cursor: 'pointer', boxShadow: '0 4px 6px -1px rgba(0, 0, 0, 0.1)', minHeight: d.amount > 0 ? '4px' : '0'}}></div>
